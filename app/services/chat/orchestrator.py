...
class ChatOrchestrator:
    ...
    async def process_message(self, question: str, session_id: str = None) -> Tuple[str, str]:
        """
        Xử lý tin nhắn chính.
        Trả về (câu trả lời, session_id).
        """
        # 1. Init Session
        if not session_id:
        ...
        if intent == "DATABASE_QUERY":
            ...
            if not current_branch:
                missing_info.append("Chi nhánh (Hà Nội, Sài Gòn, Đà Nẵng)")
            if not current_grade:
                missing_info.append("Khối (10, 11, 12)")
            ...
            if missing_info:
                ...
                return f"Để tư vấn chính xác, thầy/cô cần biết em đang quan tâm đến {info_req} nào? Em vui lòng cung cấp thêm nhé.", session_id
